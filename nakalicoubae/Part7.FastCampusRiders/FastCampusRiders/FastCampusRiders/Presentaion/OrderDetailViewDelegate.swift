...
class OrderDetailViewDelegate: NSObject {
    ...
    init(
        ...
        self.detailInfoList = OrderDetailViewDelegate.setupTableViewData(
...
extension OrderDetailViewDelegate {
    func drawRoutes(_ routes: [CLLocation]?) {
        self.detailInfoList = OrderDetailViewDelegate.setupTableViewData(
            detailInfo: self.OrderDetailInfo,
            routes: routes
        )
    }
    
    private static func setupTableViewData(
        detailInfo: OrderDetailInfo,
        routes: [CLLocation]?
    ) -> [OrderDetailCellType] {
        ...
        detailInfoType.append(contentsOf: [
            .foldableCell(
                title: "픽업지 정보",
                cellInfo: .storeInfo(
                    locationInfo: detailInfo.storeInfo,
                    tintColor: storeTintColor
                ),
                isFolded: false,
            ...
            .foldableCell(
                title: "전달지 정보",
                cellInfo: .destinationInfo(
                    locationInfo: detailInfo.destinationInfo,
                    tintColor: destinationTintColor
                ),
                isFolded: false,
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .map(let detailInfo, let routes):
            let cell = tableView.dequeueReusableCell(withIdentifier:
                                                        OrderDetailMapViewCell.cellIdentifier,
                                                     for: indexPath)
...
//                }
        ...
        case .description(let detailInfo):
            let cell = tableView.dequeueReusableCell(withIdentifier:
                                                        OrderDetailDescriptionCell.cellIdentifier,
                                                     for: indexPath)
        ...
        case .foldableCell(let title, let cellInfo, let  isFolded, let textColor):
            if indexPath.row == 0 {
                let cell = tableView.dequeueReusableCell(
                    withIdentifier: OrderDetailTitleCell.cellIdentifier,
                    for: indexPath
                )
                ...
                case .memo(let memo):
                    let cell = tableView.dequeueReusableCell(
                        withIdentifier: OrderDetailMemoCell.cellIdentifier,
                        for: indexPath
                    )
                ...
                case .storeInfo(let locationInfo, let tintColor),
                    ...
                    let cell = tableView.dequeueReusableCell(
                        withIdentifier: OrderDetailContactCell.cellIdentifier,
                        for: indexPath
                    )
                ...
                case .receiptInfo(let detailInfo):
                    let cell = tableView.dequeueReusableCell(
                        withIdentifier: OrderDetailReceiptCell.cellIdentifier,
                        for: indexPath
                    )
