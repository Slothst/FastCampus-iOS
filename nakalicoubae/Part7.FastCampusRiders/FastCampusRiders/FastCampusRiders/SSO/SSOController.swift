...
class SSOController {
    ...
    private init() {
        ...
        guard let appID = mainBundle.object(forInfoDictionaryKey: "appID") as? String else {
            assertionFailure("noAppID")
            return
        }
        self.appID = appID
        
        guard let appSecret = mainBundle.object(forInfoDictionaryKey: "appSecret") as? String else {
...
extension SSOController {
    ...
    private func requestToken(_ status: SSOStatus) async -> Result<SSOStatus, Error> {
        ...
        let apiPath = APIController.Path.issueToken(
            appID: appID,
            secret: appSecret,
            id: id,
            password: password
        ...
        return await withCheckedContinuation { continuation in
            result.fold { tokenInfo in
                continuation.resume(returning: .success(.requestUserInfo(
    ...
    private func refreshToken(_ status: SSOStatus) async -> Result<SSOStatus, Error> {
        ...
        let apiPath = APIController.Path.refreshToken(
            appID: appID,
            secret: appSecret,
    ...
    private func requestUserInfo(_ status: SSOStatus) async -> Result<SSOStatus, Error> {
        ...
        let apiPath = APIController.Path.readUserInfo(
            appID: appID,
            secret: appSecret,
