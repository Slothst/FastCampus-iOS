...
final class UserDBRepositoryTests: XCTestCase {
    ...
    func test_getUser_success() {
        ...
        let userDBRepository = UserDBRepository(reference: StubUserDBRepository(value: stubData))
    ...
    func test_getUser_isEmpty() {
        let userDBRepository = UserDBRepository(reference: StubUserDBRepository(value: nil))
...
struct StubUserDBRepository: DBReferenceType {
    ...
    func fetch(key: String, path: String?) -> AnyPublisher<Any?, LMessenger.DBError> {
