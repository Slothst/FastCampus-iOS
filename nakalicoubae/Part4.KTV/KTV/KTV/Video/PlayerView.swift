...
class PlayerView: UIView {
    ...
    func set(url: URL) {
        self.player = AVPlayer(playerItem: AVPlayerItem(asset: AVURLAsset(url: url)))
    }
    ...
    private func setup(player: AVPlayer) {
        self.playObservation = player.addPeriodicTimeObserver(
            ...
            guard let currentItem = player?.currentItem,
                  ...
                  let timeRanges = (currentItem.loadedTimeRanges as? [CMTimeRange])?.first else {
                return
            }
        ...
        self.statusObservation = player.currentItem?.observe(
            \.status,
             changeHandler: { [weak self] item, _ in
                 ...
                 case .failed, .unknown:
                     print("failed to play \(String(describing: item.error?.localizedDescription ?? ""))")
                 @unknown default:
                     print("failed to play \(String(describing: item.error?.localizedDescription ?? ""))")
                 }
             }
